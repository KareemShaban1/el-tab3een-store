...
$is_superadmin = auth()->user()->can('superadmin');
        ...
        Menu::create('admin-sidebar-menu', function ($menu) use ($is_superadmin) {
            ...
            //Purchase dropdown
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update')) || $is_superadmin) {
                $menu->dropdown(
            ...
            if (in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('stock_transfer.view') || auth()->user()->can('stock_transfer.create') || auth()->user()->can('stock_transfer.view_own')) || $is_superadmin) {
                $menu->dropdown(
            ...
            if (in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('stock_adjustment.view') || auth()->user()->can('stock_adjustment.create') || auth()->user()->can('view_own_stock_adjustment')) || $is_superadmin) {
                $menu->dropdown(
            ...
            //Expense dropdown
            if (in_array('expenses', $enabled_modules) && (auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) || $is_superadmin) {
                $menu->dropdown(
                    __('expense.expenses'),
                    function ($sub) {
                        $sub->url(
            ...
            //Accounts dropdown
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules) || $is_superadmin) {
                $menu->dropdown(
            ...
            //Reports dropdown
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view') || $is_superadmin
                ...
                $menu->dropdown(
                    __('report.reports'),
                    function ($sub) use ($enabled_modules, $is_admin, $is_superadmin) {
