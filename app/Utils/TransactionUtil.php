...
class TransactionUtil extends Util
{
        ...
        if ($only_count) {
            ...
            $sells->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
                ...
                ->select(
                    ...
                    'transactions.order_addresses',
                    'transactions.prefer_payment_method',
                    'contacts.name',
                    'contacts.mobile',
                    'contacts.contact_id',
                    'contacts.supplier_business_name',
                    ...
                    'transactions.ecommerce_order_status',
                    'transactions.payment_status',
                    'transactions.final_total',
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        if ($sell->source === 'ecommerce') {
            $sell_before = $sell->replicate();
            $sell->ecommerce_order_status = 'refunded';
            $sell->shipping_status = 'refunded';
            $sell->save();

            $this->activityLog($sell, 'edited', $sell_before);
        }
