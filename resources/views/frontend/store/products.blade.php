...
if ($sPmax <= $sPmin) { $sPmax=$sPmin + 1; } $sliderRngLo=$sPmin; $sliderRngHi=$sPmax; if (request()->
	filled('price_min')) {
	...
	.store-products-pagination.is-loading {
		opacity: .55;
		pointer-events: none;
	}
	...
	<div class="container products-wrap">
		...
		<aside class="card filters-card">
			...
			<div class="active-filters js-store-active-filters">
				...
				$activeSubCategory = $sub_categories->firstWhere('id', (int)
				$activeSubCategoryId);
				...
				<a class="filter-pill js-store-ajax-nav"
					href="{{ route('store.products.index', $queryWithoutSubCategory) }}"
			...
			<form method="GET" action="{{ route('store.products.index') }}"
				class="js-store-filter-form" id="store-products-filters-desktop" novalidate>
				@if($isServoCatalog)
				...
				<div class="filter-group">
					...
					<div class="search-wrap">
						<input id="filter-q-desktop" type="text" name="q"
							value="{{ request('q') }}"
							placeholder="{{ __('lang_v1.product_name') }}">
						@if($activeQ !== '')
						<a class="clear-q-btn js-store-ajax-nav"
							href="{{ route('store.products.index', $queryWithoutQ) }}"
				...
				<div class="filter-group">
					<label
						for="filter-sub-category-desktop">{{ __('product.sub_category') }}</label>
					<select id="filter-sub-category-desktop" name="sub_category_id"
						data-all-label="{{ __('lang_v1.all') }}">
						<option value="">{{ __('lang_v1.all') }}</option>
						@foreach(($sub_categories ?? collect()) as $subCategory)
						@if(empty($activeCategoryId) || (string)
						$subCategory->parent_id === (string) $activeCategoryId)
						<option value="{{ $subCategory->id }}"
							@selected((string)$activeSubCategoryId===(string)$subCategory->
				...
				<div class="filter-group filter-group--price-slider">
					...
					<div class="price-dual-range">
						<div class="price-dual-range__values"
							id="price-slider-label-desktop"></div>
						<div class="price-dual-range__rail">
							<div class="price-dual-range__track-bg"></div>
							<div class="price-dual-range__fill"
								id="price-fill-desktop"></div>
							<input type="range"
								class="price-dual-range__input price-dual-range__input--min"
								id="range-price-min-desktop"
								min="{{ $sPmin }}" max="{{ $sPmax }}"
								step="{{ $sPstep }}"
								value="{{ $sliderRngLo }}">
							<input type="range"
								class="price-dual-range__input price-dual-range__input--max"
								id="range-price-max-desktop"
								min="{{ $sPmin }}" max="{{ $sPmax }}"
								step="{{ $sPstep }}"
								value="{{ $sliderRngHi }}">
						</div>
					</div>
					<input type="hidden" name="price_min" id="filter-price-min-desktop"
						value="{{ $hiddenPriceMin }}">
					<input type="hidden" name="price_max" id="filter-price-max-desktop"
		...
		<main data-store-products-main>
			<div class="card">
				...
				<div class="products-grid">
					...
					$vars = collect($item['variations'] ?? []);
					$defVar = $vars->first();
					$defaultPrice = $defVar ? (float) ($defVar['price_inc_tax'] ?? 0) :
					(float) ($item['min_price'] ?? 0);
					$defVid = $defVar ? (int) ($defVar['variation_id'] ?? 0) : (int)
					($item['variation_id'] ?? $item['id']);
					...
					<div class="prod-card">
						...
						<div class="prod-info">
							<div class="prod-brand">
								{{ $item['brand'] ?: 'Brand' }}</div>
							<div class="prod-name">
								<a href="{{ $productShowUrl }}"
									title="عرض تفاصيل المنتج">{{ $item['name'] }}</a>
							</div>
							...
							<div class="prod-variant-wrap">
								<select class="prod-variant"
									data-id="{{ (int) $item['id'] }}">
									@foreach($vars as $v)
									<option value="{{ (int) ($v['variation_id'] ?? 0) }}"
										data-price="{{ (float) ($v['price_inc_tax'] ?? 0) }}">
										{{ $v['name'] ?: 'Default' }}
										—
										{{ number_format((float) ($v['price_inc_tax'] ?? 0), 2) }}
									</option>
									@endforeach
								</select>
							</div>
					...
					<div class="prod-card">
						<div class="prod-info">
							<div class="prod-name">لا توجد منتجات مطابقة
								للفلاتر.</div>
						</div>
					</div>
					@endforelse
				...
				<div class="store-products-pagination">
					@if(!empty($products) && method_exists($products, 'hasPages') &&
					$products->hasPages())
					{!! $products->links() !!}
					@endif
				</div>
	...
	<div id="filter-sheet" class="filter-sheet">
		...
		<form method="GET" action="{{ route('store.products.index') }}" class="js-store-filter-form"
			id="store-products-filters-mobile" novalidate>
			@if($isServoCatalog)
			<input type="hidden" name="source" value="servo">
			@endif
			<div class="filter-group">
				...
				<div class="search-wrap">
					<input id="filter-q-mobile" type="text" name="q"
						value="{{ request('q') }}"
						placeholder="{{ __('lang_v1.product_name') }}">
					@if($activeQ !== '')
					<a class="clear-q-btn js-store-ajax-nav"
			...
			<div class="filter-group">
				...
				<select id="filter-sub-category-mobile" name="sub_category_id"
					data-all-label="{{ __('lang_v1.all') }}">
					<option value="">{{ __('lang_v1.all') }}</option>
					@foreach(($sub_categories ?? collect()) as $subCategory)
					@if(empty($activeCategoryId) || (string) $subCategory->parent_id ===
			...
			<div class="filter-group filter-group--price-slider">
				<label>{{ __('lang_v1.price_range') }}</label>
				<div class="price-dual-range">
					<div class="price-dual-range__values" id="price-slider-label-mobile">
					</div>
					<div class="price-dual-range__rail">
						<div class="price-dual-range__track-bg"></div>
						<div class="price-dual-range__fill" id="price-fill-mobile">
						</div>
						<input type="range"
							class="price-dual-range__input price-dual-range__input--min"
							id="range-price-min-mobile" min="{{ $sPmin }}"
							max="{{ $sPmax }}" step="{{ $sPstep }}"
							value="{{ $sliderRngLo }}">
						<input type="range"
							class="price-dual-range__input price-dual-range__input--max"
							id="range-price-max-mobile" min="{{ $sPmin }}"
							max="{{ $sPmax }}" step="{{ $sPstep }}"
							value="{{ $sliderRngHi }}">
					</div>
				</div>
				<input type="hidden" name="price_min" id="filter-price-min-mobile"
					value="{{ $hiddenPriceMin }}">
				<input type="hidden" name="price_max" id="filter-price-max-mobile"
					value="{{ $hiddenPriceMax }}">
			</div>
			<div class="filter-actions">
				<button class="btn" type="submit">{{ __('lang_v1.apply') }}</button>
	...
	(function() {
		...
		function bindPriceSliders() {
			['desktop', 'mobile'].forEach((suffix) => {
				['range-price-min-' + suffix, 'range-price-max-' + suffix]
				.forEach((id) => {
					const el = document
						.getElementById(
						id);
		...
		function resetAllPriceSliders() {
			const b = getPriceSliderBounds();
			['desktop', 'mobile'].forEach((suffix) => {
				const lo = document.getElementById('range-price-min-' +
					suffix);
				const hi = document.getElementById('range-price-max-' +
					suffix);
				const hMi = document.getElementById('filter-price-min-' +
					suffix);
				const hMa = document.getElementById('filter-price-max-' +
					suffix);
		...
		function fillSubCategorySelect(select, parentId, selectedId) {
			...
			STORE_SUBCATEGORIES.forEach((sub) => {
				...
				options.push('<option value="' + id + '"' + (id ===
						selected ? ' selected' : ''
						) + '>' + escHtml(sub.name) +
					'</option>');
			});
		...
		function syncSubCategorySelect(fromSuffix, toSuffix) {
			...
			fillSubCategorySelect(toSub, fromCategory ? fromCategory.value : '', fromSub ? fromSub
				.value : '');
		}
		...
		function fillFormsFromParams(p) {
			...
			if (!subCategoryId && categoryId) {
				const asSub = STORE_SUBCATEGORIES.find((sub) => String(sub.id) === String(
					categoryId));
				if (asSub) {
		...
		function renderPills(filters) {
			...
			if (f.q) {
				parts.push(
					`<a class="filter-pill js-store-ajax-nav" href="${urlWithoutFilterKey(f, 'q')}"><span class="pill-label">Search</span><span>“${String(f.q).replace(/</g, '&lt;')}</span><span class="pill-x">✕</span></a>`);
			}
			if (f.category_id) {
				let name = STORE_CATEGORY_NAMES[f.category_id] || STORE_CATEGORY_NAMES[String(f
					.category_id)] || '';
				...
				if (name) {
					...
					const href = p.toString() ? (STORE_PRODUCTS_URL + '?' + p
					.toString()) : STORE_PRODUCTS_URL;
					parts.push(
			...
			if (f.brand_id && f.source !== 'servo') {
				...
				if (name) {
					parts.push(
						`<a class="filter-pill js-store-ajax-nav" href="${urlWithoutFilterKey(f, 'brand_id')}"><span class="pill-label">Brand</span><span>${String(name).replace(/</g, '&lt;')}</span><span class="pill-x">✕</span></a>`);
				}
			}
			if (f.price_min || f.price_max) {
				...
				const href = p.toString() ? (STORE_PRODUCTS_URL + '?' + p.toString()) :
					STORE_PRODUCTS_URL;
				parts.push(
		...
		function syncProductsFromGridItems(items) {
			...
			(items || []).forEach((p) => {
				const variations = Array.isArray(p.variations) ? p
					.variations : [];
				const defaultVariant = variations[0] || null;
				const defaultPrice = Number(defaultVariant ? defaultVariant
					.price_inc_tax : (p.min_price || p
						.price || 0));
				PRODUCTS[p.id] = {
					name: p.name,
					brand: p.brand || '',
					...
					// stars: '⭐⭐⭐⭐⭐',
					reviews: 'متوفر',
					variation_id: defaultVariant ? Number(
							defaultVariant
							.variation_id) :
						Number(p.variation_id || p
							.id),
					variations,
					...(p.source === 'servo' ? {
						source: 'servo'
		...
		function renderGrid(items) {
			if (!items || !items.length) {
				grid.innerHTML =
					'<div class="prod-card"><div class="prod-info"><div class="prod-name">لا توجد منتجات مطابقة للفلاتر.</div></div></div>';
				return;
			}
			grid.innerHTML = items.map((p, idx) => {
				...
				const defaultPrice = Number(defaultVariant ? defaultVariant
					.price_inc_tax : (p.min_price || p
						.price || 0));
				const defVid = defaultVariant ? Number(defaultVariant
					.variation_id) : Number(p
					.variation_id || p.id);
		...
		async function fetchStoreProducts(url) {
			...
			try {
				...
				if (metaTotal) {
					metaTotal.textContent = 'Total: ' + (json.meta && json.meta
						.total != null ? json.meta.total : 0);
				}
				if (pagWrap) {
		...
		document.addEventListener('click', (e) => {
			...
			const pageA = e.target.closest(
				'.store-products-pagination a.page-link');
