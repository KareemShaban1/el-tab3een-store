	...
	<div class="repair-status-card">
		...
		<form id="check-repair-status-form" method="POST" action="{{ route('post-repair-status') }}">
			...
			<div class="repair-field">
				<label
					for="repair-search-type">{{ __('storefront.repair_status.search_type') }}</label>
				<div class="repair-search-grid">
					...
					<input id="repair-search-number" type="text" name="search_number"
			...
			<div class="repair-field">
				...
				<input id="repair-serial-no" type="text" name="serial_no"
					placeholder="{{ __('repair::lang.serial_no') }}">
			</div>

			<div class="repair-search-actions">
				<button class="btn" type="submit"
					id="repair-search-btn">{{ __('lang_v1.search') }}</button>
				<a href="{{ route('welcome') }}"
					class="repair-btn-secondary">{{ __('storefront.repair_status.back_to_store') }}</a>
			</div>
...
(function() {
	...
	form?.addEventListener('submit', async function(e) {
		...
		try {
			...
			if (result.success) {
				resultsWrap.innerHTML = result
					.repair_html || '';
				if (typeof toast === 'function') {
					toast(result.msg || REPAIR_STATUS_MSG
						.results_found
						);
				}
			} else {
			...
			resultsWrap.innerHTML = '';
			if (typeof toast === 'function') {
				toast(REPAIR_STATUS_MSG.search_error,
					'error');
			}
		} finally {
