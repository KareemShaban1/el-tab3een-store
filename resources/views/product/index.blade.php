...
<section class="content">
		...
		<div class="col-md-12">
			...
			<div class="col-md-3">
				<div class="form-group">
					...
					{!! Form::select(
					'type',
					['single' => __('lang_v1.single'), 'variable' =>
					__('lang_v1.variable'), 'combo' => __('lang_v1.combo')],
					null,
					[
			...
			<div class="col-md-3">
				...
				<div class="form-group">
					{!! Form::select(
					'active_state',
					['active' => __('business.is_active'), 'inactive' =>
					__('lang_v1.inactive')],
					null,
					[
					'class' => 'form-control select2',
			...
			<div class="col-md-3">
				<div class="form-group">
					<br>
					<label>
						{!! Form::checkbox('not_for_selling', 1, false, ['class' =>
						'input-icheck', 'id' => 'not_for_selling']) !!}
			...
			<div class="col-md-3">
				<div class="form-group">
					<br>
					<label>
						{!! Form::checkbox('woocommerce_enabled', 1, false, ['class'
						=> 'input-icheck', 'id' => 'woocommerce_enabled']) !!}
	...
	<div class="row">
		<div class="col-md-12 tw-mb-2">
			<button type="button" id="btn_low_stock_vld_modal"
				class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-warning tw-font-semibold">
				<i class="fa fa-exclamation-triangle"></i>
				@lang('lang_v1.low_stock_by_location_button')
			</button>
		</div>
	</div>
		...
		<div class="col-md-12">
			...
			<div class="nav-tabs-custom">
				<ul class="nav nav-tabs">
					<li class="active">
						<a href="#product_list_tab" data-toggle="tab"
							aria-expanded="true"><i class="fa fa-cubes"
								aria-hidden="true"></i>
							@lang('lang_v1.all_products')</a>
					</li>
					@can('stock_report.view')
					<li>
						<a href="#product_stock_report" class="product_stock_report"
	...
	@can('product.update')
	<div class="modal fade" id="low_stock_vld_modal" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">@lang('lang_v1.low_stock_by_location_title')
					</h4>
				</div>
				<div class="modal-body">
					...
					<div id="low_stock_vld_empty" class="alert alert-info hide tw-mb-0">
						@lang('lang_v1.low_stock_by_location_none')
					</div>
					<div id="low_stock_vld_table_wrap" class="table-responsive hide">
						<table class="table table-bordered table-condensed table-striped"
							id="low_stock_vld_table">
							<thead>
								<tr>
									<th>@lang('sale.product')</th>
									<th>@lang('product.variations')
									</th>
									<th>@lang('purchase.business_location')
									</th>
							...
							<tbody id="low_stock_vld_tbody"></tbody>
						</table>
					</div>
				</div>
				<div class="modal-footer">
					<button type="button"
						class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-neutral"
						data-dismiss="modal">@lang('messages.close')</button>
					<button type="button" class="tw-dw-btn tw-dw-btn-primary"
						id="low_stock_vld_save" disabled>
...
$(document).ready(function() {
	...
	$('table#product_table tbody').on('click',
		'.product-flag-toggle, .product-flag-toggle-wrap',
		function(e) {
			e.stopPropagation();
		});
	...
	$(document).on('submit', 'form#toggle_woocommerce_sync_form', function(e) {
		e.preventDefault();
		var url = $('form#toggle_woocommerce_sync_form').attr(
			'action');
		var method = $('form#toggle_woocommerce_sync_form').attr(
			'method');
		var data = $('form#toggle_woocommerce_sync_form')
			.serialize();
		...
		$.ajax({
			...
			success: function(result) {
				...
				if (result
					...
					$("input#woocommerce_products_sync")
						.val(
							''
						);
					$('#woocommerce_sync_modal')
						.modal(
							'hide'
						);
					toastr.success(result
		...
		if (div.length) {
			$.ajax({
				url: "{{ action([\App\Http\Controllers\ReportController::class, 'getStockReport']) }}" +
					'?for=view_product&product_id=' + div
					.data('product_id'),
				dataType: 'html',
				success: function(result) {
					div.html(result);
		...
		if (!data_table_initailized) {
			...
			stock_report_table = $('#stock_report_table').DataTable({
				...
				"footerCallback": function(row, data,
					...
					var footer_total_stock =
						0;
					var footer_total_sold =
						0;
					var footer_total_transfered =
						0;
					var total_adjusted =
						0;
					var total_stock_price =
						0;
					var footer_stock_value_by_sale_price =
						0;
					var total_potential_profit =
						0;
					var footer_total_mfg_stock =
						0;
					for (var r in
							data) {
						footer_total_stock
					...
					$('.footer_total_stock')
						.html(__currency_trans_from_en(
							footer_total_stock,
							false
						));
					$('.footer_total_stock_price')
						.html(__currency_trans_from_en(
							total_stock_price
						));
					$('.footer_total_sold')
					...
					$('.footer_potential_profit')
						.html(__currency_trans_from_en(
							total_potential_profit
						));
					if ($('th.current_stock_mfg')
						.length) {
						$('.footer_total_mfg_stock')
							.html(__currency_trans_from_en(
								footer_total_mfg_stock,
	...
	$.ajax({
		...
		success: function(result) {
			if (result.success == true) {
				$('div#edit_product_location_modal')
					.modal('hide');
...
function loadLowStockVldModal() {
	...
	$('#low_stock_vld_table_wrap').addClass('hide');
	$('#low_stock_vld_empty').addClass('hide');
	$('#low_stock_vld_save').prop('disabled', true);
	$('#low_stock_vld_tbody').empty();

	$.ajax({
		url: "{{ action([\App\Http\Controllers\ProductController::class, 'getLowStockLocationRows']) }}",
		dataType: 'json',
		success: function(r) {
			$('#low_stock_vld_loading').addClass('hide');
			if (!r.success || !r.rows || !r.rows.length) {
				$('#low_stock_vld_empty').removeClass('hide');
				return;
			}
			var html = '';
			r.rows.forEach(function(row) {
				...
				html += '<td>' + esc(row
					...
					'</td>';
				html += '<td><input type="text" class="form-control input-sm input_number input_quantity low-stock-vld-qty" value="' +
					esc(row
						.qty_display
					) +
					'" /></td>';
			...
			$('#low_stock_vld_tbody').html(html);
			$('#low_stock_vld_table_wrap').removeClass('hide');
			$('#low_stock_vld_save').prop('disabled', false);
...
$(document).on('click', '#btn_low_stock_vld_modal', function() {
	$('#low_stock_vld_modal').modal('show');
	loadLowStockVldModal();
});

$(document).on('click', '#low_stock_vld_save', function() {
	var btn = $(this);
	var items = [];
	$('#low_stock_vld_tbody tr').each(function() {
		var id = $(this).data('vld-id');
		var inp = $(this).find('input.low-stock-vld-qty');
		var qty = typeof __read_number === 'function' ?
			__read_number(inp) : parseFloat(String(inp.val())
				.replace(/,/g, '')) || 0;
		items.push({
			id: id,
			qty_available: qty
	...
	$.ajax({
		method: 'POST',
		url: "{{ action([\App\Http\Controllers\ProductController::class, 'bulkUpdateVldQuantities']) }}",
		dataType: 'json',
